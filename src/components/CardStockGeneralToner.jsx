...
function CardStockGeneralToner() {
  ...
  useEffect(() => {
    fetch(`${backendUrl}/api/toners`)
      .then((res) => res.json())
      .then((data) => {
        const tonersEnAlmacen = data.filter(
          (toner) => toner.ubicacion === "Almacen"
        );

        setStockTotal(tonersEnAlmacen.length);

        const conteoModelos = tonersEnAlmacen.reduce((acc, toner) => {
          acc[toner.modelo] = (acc[toner.modelo] || 0) + 1;
          return acc;
        }, {});
  ...
  return (
    <>
      ...
      >
        <FaTint className="text-blue-600 text-4xl mb-3" />
        <h2 className="text-lg font-semibold text-gray-700">Stock General</h2>
        <p className="text-gray-500 text-sm">Total de toners en almacén</p>
...
export default CardStockGeneralToner;
