    ...
    fetch(`${backendUrl}/api/unidades-imagen`)
      ...
      .then((data) => {
        const unidadesImgEnAlmacen = data.filter(
          (unidadImg) => unidadImg.ubicacion === "Almacen"
        );

        setStockTotal(unidadesImgEnAlmacen.length);

        const conteoModelos = unidadesImgEnAlmacen.reduce((acc, toner) => {
  ...
  return (
    <>
      ...
      >
        ...
        <p className="text-gray-500 text-sm">Total de unidades de imagen en almacén</p>
...
export default CardStockGeneralUnidadesImg;
