...
function CardEntregadosMesToner() {
  ...
  useEffect(() => {
    fetch(`${backendUrl}/api/toners`)
      ...
      .catch((error) =>
        console.error("Error al obtener tóneres entregados:", error)
      );
  }, []);
  ...
  return (
    <div className="bg-white shadow-lg rounded-lg p-6 flex flex-col items-center text-center">
      ...
      <p className="text-gray-500 text-sm">Toners entregados en abril</p>
      <span className="text-3xl font-bold text-green-700 mt-2">{cantidad}</span>
    </div>
  );
}

export default CardEntregadosMesToner;
