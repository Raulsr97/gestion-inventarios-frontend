...
import CardStockGeneralToner from "../components/CardStockGeneralToner";
import CardMovimientosMesToner from "../components/CardMovimientosToner";
...
function ConsultaToner() {
  ...
  useEffect(() => {
    fetch(`${backendUrl}/api/toners`)
      ...
      .catch((error) => {
        console.error("❌ Error al obtener toners:", error.message);
      });
  }, []);
  ...
  const exportarAExcel = (datos) => {
    ...
    const rows = datos.map((toner) => [
      toner.serie,
      toner.modelo,
      toner.tipo,
      toner.ubicacion,
      toner.cliente?.nombre || "SIN CLIENTE",
      toner.proyecto?.nombre || "SIN PROYECTO",
      toner.proveedor?.nombre || "SIN PROVEEDOR",
      toner.marca?.nombre || "SIN MARCA",
      toner.fecha_entrada ? new Date(toner.fecha_entrada).toLocaleDateString() : "NO REGISTRADA",
      toner.fecha_salida ? new Date(toner.fecha_salida).toLocaleDateString() : "NO REGISTRADA",
      toner.fecha_entrega_final ? new Date(toner.fecha_entrega_final).toLocaleDateString() : "NO REGISTRADA",
      toner.numero_remision || "-"
    ]);
    ...
    XLSX.utils.book_append_sheet(wb, ws, "REPORTE DE TONERS");
    XLSX.writeFile(wb, "reporte_toners.xlsx");
  };
  ...
  const modelosUnicos = [...new Set(toners.map((t) => t.modelo))];
  const tiposUnicos = [...new Set(toners.map((t) => t.tipo))];
  const clientesUnicos = [...new Set(toners.map((t) => t.cliente?.nombre).filter(Boolean))];
  const proyectosUnicos = [...new Set(toners.map((t) => t.proyecto?.nombre).filter(Boolean))];
  const proveedoresUnicos = [...new Set(toners.map((t) => t.proveedor?.nombre).filter(Boolean))];
  const marcasUnicas = [...new Set(toners.map((t) => t.marca?.nombre).filter(Boolean))];
    ...
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
          <CardStockGeneralToner />
          <CardMovimientosMesToner />
          <CardEnTransitoToner />
          <CardEntregadosMesToner />
        </div>
                  ...
                  {resultados.map((t, i) => (
                    <tr key={i} className="hover:bg-gray-100">
                      ...
                      <td className="border border-gray-300 p-2 text-center">
                        ...
                        t.relacion_toners?.[0]?.remision?.numero_remision ? (
                          <a
                            href={`/remisiones/buscar?numero_remision=${t.relacion_toners[0].remision.numero_remision}`}
